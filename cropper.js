...
(function(){
  ...
  function ensureCropStyles(){
    ...
    s.textContent=`
      ...
      .pack-type{font-size:9px!important}
      .pack-name{font-size:14px!important;line-height:1.25!important;margin-top:2px!important}
      .pack-note{font-size:10px!important;margin-top:3px!important}
      .pack-card-actions{gap:4px!important;margin-top:8px!important}
      .pack-card-actions button{width:28px!important;height:28px!important;border-radius:9px!important}
      .pack-check{gap:5px!important;font-size:10px!important}
      .pack-check input{width:16px!important;height:16px!important}
      .photo-preview{aspect-ratio:1/1!important;height:auto!important;background:#f3efec!important}
      .photo-preview img{width:100%!important;height:100%!important;object-fit:contain!important;object-position:center!important}
      ...
      .voya-crop-stage{position:relative;width:100%;aspect-ratio:1/1;background:#f2ece7;border-radius:20px;overflow:hidden;touch-action:none;user-select:none}
      ...
      .voya-crop-grid{position:absolute;inset:0;pointer-events:none;background:linear-gradient(to right,transparent 33.1%,rgba(255,255,255,.55) 33.3%,rgba(255,255,255,.55) 33.7%,transparent 33.9%,transparent 66.1%,rgba(255,255,255,.55) 66.3%,rgba(255,255,255,.55) 66.7%,transparent 66.9%),linear-gradient(to bottom,transparent 33.1%,rgba(255,255,255,.55) 33.3%,rgba(255,255,255,.55) 33.7%,transparent 33.9%,transparent 66.1%,rgba(255,255,255,.55) 66.3%,rgba(255,255,255,.55) 66.7%,transparent 66.9%);box-shadow:inset 0 0 0 1px rgba(255,255,255,.8);border-radius:20px}
  ...
  function openCropper(dataUrl){
    ...
    const html=`<div class="voya-crop-backdrop"><div class="voya-crop-card" onclick="event.stopPropagation()">
      <div class="voya-crop-head"><h3>${cropText('Adjust photo','调整图片')}</h3><button type="button" onclick="voyaCloseCropper()">×</button></div>
      <div class="voya-crop-stage" id="voyaCropStage"><img id="voyaCropImage" src="${dataUrl}" alt=""><div class="voya-crop-grid"></div></div>
      <div class="voya-crop-help">${cropText('The full photo is shown first · Drag or zoom if you want','默认完整显示 · 可拖动或缩放调整')}</div>
    ...
    img.onload=()=>{
      ...
      // Fit the whole image inside the 1:1 frame by default instead of cropping it.
      cropState.baseScale=Math.min(side/img.naturalWidth,side/img.naturalHeight);
      ...
      zoom.max=String(cropState.baseScale*4);
    ...
    stage.addEventListener('pointermove',e=>{
      ...
      if(pts.length===1){cropState.x+=e.clientX-prev.x;cropState.y+=e.clientY-prev.y;applyTransform()}
        ...
        if(cropState.lastPinch){cropState.scale=Math.min(cropState.baseScale*4,Math.max(cropState.baseScale,cropState.scale*(d/cropState.lastPinch)));cropState.zoom.value=String(cropState.scale);applyTransform()}
