...
function addPackPreset(encodedType){const tr=currentTrip();if(!tr)return;const type=decodeURIComponent(encodedType||'').trim();if(!type)return;tr.packing.push({id:uid(),name:type,type,person:packPerson,cat:packCategory,note:'',image:'',packed:false});saveStore(true);renderPacking()}
